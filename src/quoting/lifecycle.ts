import { can, type Permission, type Quote, type QuoteStatus, type Role } from '../platform/types';
 ...
 */
export type QuoteAction = 'submit' | 'pick-up' | 'request-approval' | 'approve' | 'return' | 'send' | 'win' | 'lose';
...
/** Whether this role may move this quotation this way, right now. */
export function allows(role: Role | null, quote: Pick<Quote, 'status' | 'kind'>, action: QuoteAction): boolean {
...
export const availableActions = (role: Role | null, quote: Pick<Quote, 'status' | 'kind'>) =>
  transitions.filter(t => allows(role, quote, t.action));
