...
import {lessonFor,siteLesson,remedyFor} from '../domain/learn';
import {planSite,type SitePlan} from '../geometry/site';
...
export function Connectivity({model}:{model:Model}){const {selected}=useStudio(),packId=selected.includes('/P')?selected.split('/').slice(0,2).join('/'):null;const cells=packId?model.cells.filter(n=>n.parent===packId):[];if(packId)return <svg className="graph" viewBox="0 0 260 192" role="img" aria-label={`Serpentine ${source.seriesCells}S${source.parallelCells}P connectivity`}><text x="10" y="17">{source.seriesCells}S{source.parallelCells}P · {source.seriesCells-1} intercell links</text>{model.electrical.edges.filter(e=>e.kind==='busbar'&&e.from.startsWith(packId+'/C')&&e.to.startsWith(packId+'/C')).map(e=>{const f=cells.find(n=>n.id===model.electrical.ports[e.from].node)!,t=cells.find(n=>n.id===model.electrical.ports[e.to].node)!;return <line key={e.id} x1={16+f.column!*18} y1={37+f.row!*19} x2={16+t.column!*18} y2={37+t.row!*19} stroke="#b78641" strokeWidth="2"/>;})}{cells.map(n=><circle key={n.id} cx={16+n.column!*18} cy={37+n.row!*19} r="4" fill={n.id===selected?'#a9ce52':'#497b98'}><title>{n.id}</title></circle>)}<text x="8" y="189">− C001 → C{String(source.seriesCells).padStart(3,'0')} + · positions ≠ parallel groups</text></svg>;
 const rack=model.racks.find(r=>r.id===selected)||model.racks[0],packs=model.packs.filter(p=>p.parent===rack.id);const ports:Record<string,[number,number]>={};packs.forEach((p,i)=>{ports[`${p.id}:-`]=[21+i*59,80];ports[`${p.id}:+`]=[55+i*59,80];});ports[`${rack.id}/PROTECTION:in`]=[95,120];ports[`${rack.id}/PROTECTION:out`]=[165,120];ports['DC-COMBINER:-']=[55,165];ports['DC-COMBINER:+']=[205,165];ports['PCS:-']=[55,205];ports['PCS:+']=[205,205];const edges=model.electrical.edges.filter(e=>e.kind==='hv'&&ports[e.from]&&ports[e.to]);return <svg className="graph" viewBox="0 0 260 230" role="img" aria-label="String electrical connectivity"><text x="14" y="20">{rack.id} · {packs.length} series packs</text>{packs.map((p,i)=><g key={p.id}><rect x={16+i*59} y="43" width="46" height="47" rx="3" fill="#dee7eb" stroke="#94a8b4"/><text x={20+i*59} y="63">{p.id.split('/')[1]}</text><text x={19+i*59} y="81">−     +</text></g>)}<rect x="85" y="109" width="91" height="24" fill="#eef2f4" stroke="#aabcc7"/><text x="87" y="105">Protection</text><rect x="37" y="153" width="185" height="28" fill="#e8eff2" stroke="#aabcc7"/><text x="95" y="171">Combiner</text>{edges.map(edge=>{const f=ports[edge.from],t=ports[edge.to];let d=`M ${f[0]} ${f[1]} L ${t[0]} ${t[1]}`;if(edge.to==='DC-COMBINER:-')d=`M ${f[0]} ${f[1]} H 9 V 165 H ${t[0]}`;else if(edge.to.endsWith('PROTECTION:in'))d=`M ${f[0]} ${f[1]} V 96 H 95 V 120`;else if(edge.from.endsWith('PROTECTION:out'))d=`M ${f[0]} ${f[1]} H 235 V 145 H 205 V 165`;return <path key={edge.id} d={d} fill="none" stroke="#dc803f" strokeWidth="2"><title>{edge.from} → {edge.to}</title></path>;})}<text x="49" y="202">−</text><text x="202" y="202">+</text><text x="85" y="219">External PCS</text><text x="10" y="36">{model.stats.parallelStrings} strings in parallel at combiner</text></svg>;
}

export function InspectPanel({model,atSite=false}:{model:Model;atSite?:boolean}){const {selected,focus,select,site}=useStudio(),n=model.nodes.find(n=>n.id===selected),pack=selected.includes('/P'),cell=n?.kind==='cell';
 // The site's own figures are shown above this panel; repeating one container's DC performance
 // under them would read as if it were the fleet's.
 if(atSite&&site)return <section><h3>One enclosure <span>EACH</span></h3>{([['Product',site.model],['Installed DC energy',`${(site.energyMWh/Math.max(site.units,1)).toFixed(2)} MWh`],['Rated power',`${(site.powerMW/Math.max(site.units,1)).toFixed(2)} MW`],['Outside size',`${site.enclosure.map(x=>x.toFixed(2)).join(' × ')} m`],['Footprint',`${(site.enclosure[0]*site.enclosure[2]).toFixed(1)} m²`]] as [string,string][]).map(([k,v])=><div className="data-row" key={k}><span>{k}</span><b>{v}</b></div>)}<p className="muted">{site.modelled?'Every unit on this site is identical. Double-click one to step inside it.':`Drawn at the ${site.model}'s own outside dimensions. Its interior is not modelled yet — stepping inside shows the reference 5 MWh assembly.`}</p>{site.modelled&&envelopeGap(site.enclosure,model.dimensions.required)}</section>;const data=cell?[['Voltage',`${source.cellVoltage} V`],['Capacity',`${source.cellAh} Ah`],['Energy',`${model.stats.cellEnergy.toFixed(4)} kWh`],['Electrical order',`${n.order!+1} / ${source.seriesCells}`],['Physical row / column',`${n.row!+1} / ${n.column!+1}`]]:pack?[['Model',source.packModel],['Electrical topology',`${source.seriesCells}S${source.parallelCells}P`],['Physical arrangement',`${source.rows} rows × ${source.columns} cells`],['Nominal voltage',`${model.stats.packVoltage} V`],['Capacity',`${source.packAh} Ah`],['Calculated energy',`${model.stats.packEnergy.toFixed(4)} kWh`],['Source energy label',`${source.packEnergyLabel} kWh`],['Nominal power',`${model.stats.packPower.toFixed(4)} kW`],['Current / C-rate',`${source.current} A / ${model.stats.cRate.toFixed(2)} C`]]:[['Power at max voltage',`${model.stats.powerAtMax.toFixed(2)} kW`],['Power at min voltage',`${model.stats.powerAtMin.toFixed(2)} kW`],['Aggregate current',`${model.stats.current} A`]];
 ...
 */
export function LearnPanel({model,plan}:{model:Model;plan:SitePlan|null}){
  const {selected,scope,focus}=useStudio();
  const atSite=scope==='SITE'&&!!plan;
  const lesson=atSite?siteLesson(plan):lessonFor(model,selected);
  ...
  const path=[...(plan?['SITE']:[]),'BESS',...(atSite?[]:selected.split('/').map((_,i,parts)=>parts.slice(0,i+1).join('/'))
  ...
  return <>
    ...
    <section>
      ...
      <div className="lesson-scale">{path.map(id=>
        <button key={id} className={id===(atSite?'SITE':selected)?'active':''} onClick={()=>focus(id)}>{label(id)}</button>)}
        {model.cells[0]&&!atSite&&!selected.includes('/C')&&
...
/**
 * The plot is laid out at the product's catalogue dimensions; the assembly is laid out from the
 * mechanical assumptions in the Configure tab. Where the second needs more room than the first,
 * that is worth saying out loud — and it is the assumptions that are unvalidated, not the product.
 */
const list=(parts:string[])=>parts.length<2?parts.join(''):`${parts.slice(0,-1).join(', ')} and ${parts.at(-1)}`;
function envelopeGap(product:number[],required:number[]){
  const axes=['length','height','width'];
  const over=required.map((r,i)=>[axes[i],r-product[i]] as [string,number]).filter(([,d])=>d>.05);
  if(!over.length)return null;
  return <div className="notice warning"><b>Assumed clearances exceed the product envelope</b>
    <p>The mechanical assumptions need {list(over.map(([axis,d])=>`${(d*1000).toFixed(0)} mm more ${axis}`))} than the {' '}
    catalogue dimensions of this system. Rack gap, aisle and service bay are design assumptions, not supplied figures —
    reconcile them with the manufacturer's internal arrangement before the layout is relied on.</p></div>;
}
